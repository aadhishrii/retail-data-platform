...
def orchestrate():
    ...
    def ingest_cdc():
        ...
        '''
        this is a loop that checks the status of the job every 5 seconds until it is 
        completed or failed
        when you run an api request we need to wait for the job to complete before we can 
        run the next task in the dag. so we need to check the status of the job until it is 
        completed or failed. if it is completed we can move on to the next task in 
        the dag. if it is failed we need to raise an exception and stop the dag 
        from running further.

        but if we are running from terminal we dont have to wait cause we dont move to the next task in the dag
        '''
        while True:

            job_run = ws.jobs.get_run(job_trigger.run_id) # status of the job run
